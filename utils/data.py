...
class PDBProtein(object):
    ...
    def _parse(self):
        ...
        # Process residues 
        self.residues = [r for _, r in residues_tmp.items()] # dictionary 형태인 residues_tmp.item에서 value만 사용한다는 말 (list(residue_tmp.values())이거랑 동일함.)
        for residue in self.residues: # residue center of mass 계산 (질량중심!! 그냥 단순히 좌표 평균이 아니라 무거운 원자는 더 가중치를 더 줘서 질량 중심을 찾는다 ㄷㄷ)
            sum_pos = np.zeros([3], dtype=np.float32) # [0,0,0]
            sum_mass = 0.0
            for atom_idx in residue['atoms']:
                sum_pos += self.pos[atom_idx] * self.atomic_weight[atom_idx] # atom의 좌표x질량
                sum_mass += self.atomic_weight[atom_idx] # atom의 질량
                if self.atom_name[atom_idx] in self.BACKBONE_NAMES: #backbone 좌표는 따로 저장 -> 원래 좌표도 dict에 저장되어있는 것
                    residue['pos_%s' % self.atom_name[atom_idx]] = self.pos[atom_idx] # residue[f"pos_{self.atom_name[atom_idx]}"] = self.pos[atom_idx] 이게 요즘 코드
            residue['center_of_mass'] = sum_pos / sum_mass # 질량중심 계산 완료
    ...
    def query_residues_radius(self, center, radius, criterion='center_of_mass'): # center 좌표를 기준으로 radius 거리 안에 있는 residues들을 반환
        center = np.array(center).reshape(3) # (3,) 형태의 vec으로 만듬 -> [x,y,z] 형태 보장
        selected = []
        for residue in self.residues:
            distance = np.linalg.norm(residue[criterion] - center, ord=2) # 좌표와 center까지의 euclidean 거리 계산
    ...
    def query_residues_ligand(self, ligand, radius, criterion='center_of_mass'):
        selected = []
        sel_idx = set()
        # The time-complexity is O(mn). #m: ligand 개수, n: residue 개수
        for center in ligand['pos']: # ligand['pos']는 모든 ligand의 atom 좌표 리스트
...
def parse_sdf_file(path): # ligand 처리 담당
    fdefName = os.path.join(RDConfig.RDDataDir, 'BaseFeatures.fdef') # [Feature definition 파일 로드] BaseFeatures.fdef: RDKit 기본 화학 feature 정의 파일 (H bond donor 등 정보 포함)
    factory = ChemicalFeatures.BuildFeatureFactory(fdefName) # 앞에서 로드한 파일을 기반으로 분자에서 화학적 feature를 추출할 수 있는 객체 생성
    # read mol
    if path.endswith('.sdf'):
        rdmol = Chem.MolFromMolFile(path, sanitize=False) # RDKit에서 sanitize: bond type 정리 등을 수행 (일단 읽기만 하고 sanitize는 나중에 한다는 구조)
    elif path.endswith('.mol2'):
        rdmol = Chem.MolFromMol2File(path, sanitize=False)
    else:
        raise ValueError
    Chem.SanitizeMol(rdmol) # sanitize 실행
    rdmol = Chem.RemoveHs(rdmol) # H 제거
